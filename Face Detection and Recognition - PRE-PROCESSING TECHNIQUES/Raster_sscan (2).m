...
countsum=0;
percentsum=0;
locstr='C:\Users\PRASHANTH-GL\Desktop\DATABASES\FERET\s';
...
WLetNo=2; %input('Please specify the number corresponding to required wavelet to be applied:\n1-db1,\n2-db4,\n3-sym4,\n4-bior1.3,\n5-rbio1.3,\n6-coif1\n');                              %1-db1,2-db4,3-sym4,4-bior1.3,5-rbio1.3,6-coif1
TotNoIter=10; %input('Number of iterations please: ');%Total no of iterations
switch DatbNumber
    case 2                              %For FERET
        switch WLetNo
            case 1
...
for NoOfTimes=1:TotNoIter
    ...
    for TotClasses=1:TClasses
        ...
        for TotTrImg = 1:NoOfTrImg
            ...
            FacRead=dct2(FacRead);
            K=FacRead(1:200,1:150);
            G=idct2(K);
            
            if  DatbNumber==2     %For FERET images
                FacRead=reshape(G',1,200*150);
            end
            FacRead=im2double(FacRead);
    ...
    for n=1:tests
        ...
        Testimg=dct2(Testimg);
        K=Testimg(1:200,1:150);
        G=idct2(K);
        image(Testimg);
        if DatbNumber==1
